...
void main() {
  ...
  test('un prêt importé compte comme confirmé sans ligne de confirmation SMS', () async {
  ...
  test(
      ...
      'rattaché, montants comptés dans le calcul de fin de cycle', () async {
    final groupId = await db.creerGroupe(
    ...
    await db.ajouterMembre(
      groupId: groupId, fullName: 'Fatou Traore', phoneNumber: '+2250000002');
...
''';
    ...
    expect(service.nomsIntrouvables(parsed.nomsMembresDistincts, resolus), isEmpty);
    ...
    // une répartition cohérente (formule avec-business-rules).
    final resultatCalcul = const EndOfCycleCalculator().calculer(EndOfCycleInput(
      partsByMember: [const MemberParts(memberId: 'aya', totalParts: 5)],
      partValueFcfa: 1000,
      totalInterestCollectedFcfa: totalInterets,
      totalFinesCollectedFcfa: 0,
    ));
    expect(resultatCalcul.totalInteretsAmendes, 5000);
  });

  test('un remboursement sans prêt importé correspondant est signalé, pas silencieusement perdu',
      () async {
    final groupId = await db.creerGroupe(
      name: 'Test', cycleDurationMonths: 9, meetingFrequency: 'mensuelle');
    final cycleId = await db.ouvrirCycle(
